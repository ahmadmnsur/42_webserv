...
int WebServer::createListenSocket(const std::string& host, int port) {
    ...
    if (host == "0.0.0.0" || host.empty()) {
        ...
        // Parse IP address manually instead of using forbidden inet_pton
...
void WebServer::setupSockets() {
    ...
    if (_listen_sockets.empty()) {
        ...
        // Don't use exit() - let the constructor handle the error
        return;
    }
}
...
void WebServer::handleNewConnection(int listen_sock) {
    ...
    // Convert client IP to string manually (instead of inet_ntop which is not allowed)
    unsigned char* ip_bytes = (unsigned char*)&client_addr.sin_addr.s_addr;
    std::stringstream client_ip_stream;
    client_ip_stream << (int)ip_bytes[0] << "." << (int)ip_bytes[1] << "." 
